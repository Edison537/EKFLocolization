...
def animate(step):
    ...
    #uNois 为输入的速度和yaw
    xTrue, zNois, uNois = data.update(xTrue, u)   ## Calculate the updated groundTruth(xTrue), noised observation(zNoise), and noised input(uNoise)
    if step % ekfStride == 0:
        relativeState = relativeEKF.EKF(uNois, zNois, relativeState, ekfStride) #EKF 
    
    #利用relativeState计算绝对坐标以方便显示
    xEsti = transform.calcAbsPosUseRelaPosWRTRob0(xTrue[:,0], relativeState, xTrue, numRob)#relativeState 就是xij, yij, yawij
